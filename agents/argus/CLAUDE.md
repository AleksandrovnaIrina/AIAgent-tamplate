...
3. **Чекай підтвердження.** Після показу сценарію запитай: "Текст ок? Генерую слайди 🎨" — і чекай відповіді. Якщо користувач хоче правки — редагуй і знову запитуй. Тільки після явного "ок" / "так" / "генеруй" переходь до кроків 4–5.
...
5. **ОБОВ'ЯЗКОВО** після збереження виведи JSON-блок (бот автоматично згенерує зображення):

```
---CAROUSEL_DATA---
{"slides":["текст слайду 1","текст слайду 2","текст слайду 3"],"topic":"тема"}
---END_CAROUSEL_DATA---
```

Важливо: у `slides` — тільки чистий текст кожного слайду БЕЗ номерів і префіксів "Слайд N:".

6. Якщо користувач хоче правки після генерації — редагуй, зберігай і виводь оновлений JSON-блок знову.
